...
from GsProcesses.traffic import Traffic as T
from GsProcesses.trafficManager import TrafficManager
import GsProcesses.consumerFunctions as CF
import GsProcesses.mavlinkCommands as MF
import GsProcesses.gfManager as GF
import GsProcesses.readLogFile as RT
import GsProcesses.playback as P
import GsProcesses.IcClass as I
...
def update_position(q, TM, master, mlog, forwarding):
    ...
    if master != 'NONE':
        if master != 'FILE':
            ...
            if int(time.time()-starttime_now) >= 1 and msg_list[0] is not None:
                starttime_now = time.time()
        ...
        if write_to_file and msg_list[0] is not None:
            with open('LogFiles/msg_in_{0}_{1}.txt'.format(da_formated, ac), 'a') as f:
                for i in msg_list:
                    f.write('Aircraft: ' + str(ac) + '; ' + str(i)+'\n')
        ...
        if 'NEW_AIRCRAFT' in consumer_message:
            ...
            with open(path) as f:
                ...
                message = ['AIRCRAFT', ac, 'ADD_TRAFFIC', t_id,
                           x[0], x[1], 0, 0, x[2], x[3], x[4], 0, x[5], t_id, master]
                TM.add_traffic(message)
            f.close()

...
export function addT(ac, center) {
    ...
    let marker = T.defineTrafficMarker(position, bearing, ac.id, t_id, source, t_id);

    // create the traffic object
    let time = Date.now()
    T.addTraffic(ac, t_id, center[0], center[1], 1, 0, 50, marker, emit, source, time)

    // update the map
    T.addTrafficToLayer(ac.id, marker)

    // create the panels
    T.createTrafficPanel(ac, t_id);
    T.createTrafficSummaryPanel()
...
export function clickStartTraffic() {
    ...
    let t_id = id[id.length - 2]
    let traffic = T.getTrafficById(ac, t_id)
    ...
    T.removeTrafficFromList(ac, t_id)
}
...
export function removeTraffic(ac, t) {
    let t_panel
    for (let a of AM.getAircraftList()) {
...
export function enterSaveTraffic(e) {
    if (e.key == 'Enter') {
        ...
        S.save_traffic(ac, lat, lng, alt, vel, hdg, emit, filename)
    }
}

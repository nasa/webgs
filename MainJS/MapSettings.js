...
// loading offline maps via tile server and map tile download
// TODO: Adjust this for use remotely
let ip = '0.0.0.0'
let port = '8080'
let offlineURL = 'http://' + ip + ':' + port + '/styles/klokantech-basic/{z}/{x}/{y}.png'
...
let offline = L.tileLayer(offlineURL, {
    attribution: osm_attribution,
    maxZoom: 20,
    id: 'offline'
})
...
export function setMapDisplayStyle() {
    ...
    for (let item of Object.keys(this._layers)) {
        ...
        if (this._layers[item].options.id === 'mapbox.satellite' || this._layers[item].options.id === 'mapbox.light' || this._layers[item].options.id === 'osm' || this._layers[item].options.id === 'offline') {
        ...
        } else if (this._layers[item].options.id === 'mapbox.streets' || this._layers[item].options.id === 'mapbox.outdoors') {
            document.documentElement.style.setProperty('--main-color', 'black')
            document.documentElement.style.setProperty('--main-color2', 'white')
            document.documentElement.style.setProperty('--main-background-Head', 'rgba(138, 203, 97, 0.0)')
            document.documentElement.style.setProperty('--main-background', 'rgba(138, 203, 97, 0.7)')
            document.documentElement.style.setProperty('--main-highlight-color', 'rgba(138, 203, 97, 0.8)')
            document.documentElement.style.setProperty('--main-background-input', 'gba(138, 203, 97, 0.1)')
            document.documentElement.style.setProperty('--main-shadow', '-1px -1px 1px rgba(233, 220, 220, 0.6), 1px -1px 1px rgba(233, 220, 220, 0.6), -1px 1px 1px rgba(233, 220, 220, 0.6), 1px 1px 1px rgba(233, 220, 220, 0.6)')
            line_color = 'default'
        }
    }

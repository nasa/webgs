...
define(function (require, exports, module) {
    "use strict";
    require("daadisplays/daa-displays-min/wwd/worldwind.min");
    ...
    // require("daa-displays-min/openMap/wmts-config");
    ...
    class OpenStreetMapRestLayer extends WorldWind.OpenStreetMapImageLayer {
        configureLayer(dc) {
            ...
            if (!this.xhr) {
                ...
                this.xhr.open("GET", "http://" + host + "/daadisplays/osm/WMTSCapabilities.xml", true);
                this.xhr.onreadystatechange = function () {
                    if (self.xhr.readyState === 4) {
                        if (self.xhr.status === 200) {
                            ...
                            wmtsCapabilities.serviceProvider.providerSiteUrl = "http://" + host + "/daadisplays";
                            wmtsCapabilities.contents.layer[0].resourceUrl[0].template =
                                "http://" + host + "/daadisplays/tiles.maps.eox.at/wmts/1.0.0/osm/default/WGS84/{TileMatrix}/{TileRow}/{TileCol}.jpg";
                            ...
                            console.warn("OSM retrieval failed (" + xhr.statusText + "): " + url);
                        }
                    }
                };
    ...
    class DAA_Airspace {
        ...
        constructor(opt) {
            ...
            // automatic resize of traffic symbols when zooming the map with the mouse
                    ...
                    if (i < this._traffic.length) {
                        this._traffic[i].setPosition({
                            ...
                            symbol: (traffic[i].symbol !== null || traffic[i].symbol !== undefined) ?
                                traffic[i].symbol : this._traffic[i].symbol,
                            name: (traffic[i].name !== null || traffic[i].name !== undefined) ?
                                traffic[i].name : this._traffic[i].name
                        }, {
    ...
    require("daadisplays/daa-displays-min/templates/daa-map-templates.js");

const altitudeTemplate = '<div id="{{id}}-inner" style="position:absolute; height: 649px; top:{{top}}px; left: 870px; opacity:0.8;">'
+    '<div style="position:absolute; overflow:hidden; width:92px; height:650px;">'
+        '<div id="{{id}}-spinner" style="transform:translateY(0px);">'
+            '<div class="{{id}}-tape" style="position:absolute; background-color:#333333; left:8px; width:85px; height:1400px;">'
...
+                '0'
+            '</div>'
+            '<div id="{{id}}-indicator-spinner" style="transform:translateY(11px); z-index:2; position:absolute; left:51px; width:40%; height:100%; color:white; float: right; font-size:26px; text-align: center; line-height:36px; font-family:serif;">'
+                '80 60 40 20 00'
+            '</div>'
+        '</div>'
...
const altitudeTicksTemplate = "{{#each ticks}}<div style='top:{{top}}px; position:absolute; width:100%; border-top:2px solid white; border-bottom:2px solid white;height:82px;'></div>{{/each}}";

const altitudeBandsTemplate = "{{#each segments}}<div id={{id}} from={{from}} to={{to}} style='top:{{top}}px; height:{{height}}px;{{#if ../dash}} background-image: repeating-linear-gradient(0deg,transparent,transparent 7px,{{../color}} 0px,{{../color}} 14px);{{else}} background-color:{{../color}};{{/if}} position:absolute; width:8px; left:{{left}}px;'></div>"
+ "{{/each}}";
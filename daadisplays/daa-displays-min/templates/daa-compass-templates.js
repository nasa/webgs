const compassTemplate = '<div id="{{id}}-inner" style="position:absolute; top:{{top}}px; left:{{left}}px;">'
+    '<!-- compass shade -->'
...
const compassBandsTemplate = "{{#each segments}}<div id={{id}} from={{from}} to={{to}} style='top:{{top}}px; height:{{height}}px;{{#if ../dash}} background-image: repeating-linear-gradient(0deg,transparent,transparent 7px,{{../color}} 0px,{{../color}} 14px);{{else}} background-color:{{../color}};{{/if}} position:absolute; width:8px; left:{{left}}px;'></div>"
+ "{{/each}}";

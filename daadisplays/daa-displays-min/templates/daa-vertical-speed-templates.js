const vspeedTemplate = '<div id="{{id}}-inner" style="overflow:hidden; position:absolute; height: 432px; width:60px; top:{{top}}px; left: 981px; opacity:0.8;">'
+        '<div class="trims">'
...
+            '<span class="vspeedP1" style="line-height:78px;">1</span>'
+            '<span class="vspeedP2">2</span>'
+            '<span class="vspeedP3">6</span>'
+        '</div>'
...
const vspeedBandsTemplate = "{{#each segments}}<div id={{id}} from={{from}} to={{to}} style='top:{{top}}px; height:{{height}}px;{{#if ../dash}} background-image: repeating-linear-gradient(0deg,transparent,transparent 7px,{{../color}} 0px,{{../color}} 14px);{{else}} background-color:{{../color}};{{/if}} position:absolute; width:8px; left:{{left}}px;'></div>"
+ "{{/each}}";